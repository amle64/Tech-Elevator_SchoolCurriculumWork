...
function displayProblem() {
  ...
  pContainer.querySelector('div.expression').textContent = expression;
}
...
function displayAnswers() {
  ...
  for(i = 0; i < answers.length; ++i) {
    listItems[i].textContent = answers[i];
  }
...
function updateCurrentProblem() {
  ...
  document.querySelector('.currentProblem').textContent = currentProblem;
}
...
document.addEventListener("DOMContentLoaded",() => {
  ...
  answerListItems.forEach( (element) => {
      element.addEventListener('click', (e) => {
        const answer = parseInt(e.currentTarget.textContent);
